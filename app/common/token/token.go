...
func New(id string) *YjgClaims {
	timeOut := cfg.Instance().Api.Jwt.Timeout
...
func (c *YjgClaims) CreateToken() (string, error) {
	...
	mySignKeyBytes, err := base64.URLEncoding.DecodeString(cfg.Instance().Api.Jwt.EncryptKey)
	if err != nil {
		return "", err
	}
...
func VerifyAuthToken(token string) (*Token, error) {
	mySignKey := cfg.Instance().Api.Jwt.EncryptKey
