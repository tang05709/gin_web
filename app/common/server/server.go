...
import (
	middle "festival/app/common/middleware"
...
func New(name, addr string, middleware ...gin.HandlerFunc) *Server {
	...
	s.Handler.NoRoute(middle.HandleNotFound)
	//注册路由
...
func (s *Server) Start(g errgroup.Group) {
	...
	log.Printf(`
	欢迎使用 7beGoAdm V1.0.0
	默认自动化文档地址:http://localhost%v/swagger/index.html
	后台管理:http://localhost%v/login
	官网：http://7be.cn`, s.Addr, s.Addr)
	g.Go(func() error {
